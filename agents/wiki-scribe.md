...
**Init check first:** If `wiki/` does not exist, create the full directory structure and schema before reading anything:

```bash
mkdir -p wiki/objects wiki/refs
```

Then write `wiki/schema.md` (use the schema from the Schema section above), `wiki/index.md` (empty, just a header line `# Wiki Index`), `wiki/relations.md` (empty, just `# Relations`), and `wiki/log.md` (with one bootstrap entry `## [ISO8601] init | wiki initialized`). Return `## Prior context\n(fresh wiki — no prior sessions)` and stop.
...
**Cost:** First call writes init files. Subsequent calls read-only. Fast.
...
4. Updating stale node: replace old `repo:analysis:overview` line in `wiki/index.md` with new entry
